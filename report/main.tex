...
\begin{itemize}
  ...
  \item \textbf{9 July 2026 -- finalization.} The final report, selected evidence figures, replay plots, source code, calibration files, and software checks were finalized. The deliverable was frozen as a physical actuator/embedded-control MVP plus diagnostic Myo subsystem and simulated SEA replay.
\end{itemize}
...
\section{Submission evidence package}

The submitted evidence package should contain this report, the selected photographs used in the report, the original videos of actuator motion, ESCON Studio screenshots, the exported ESCON configuration, the STM32 project, the Myo calibration/log files, and a source-code archive or link. The main report includes representative figures; the full videos are kept as companion evidence so that the actuator motion can be reviewed directly.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/stm32-escon-wiring-annotated.jpg}
\caption{Annotated STM32-to-ESCON signal wiring. Blue connects PA4/A2 to ESCON J6 pin 1 for Analog Input 1; yellow connects PC0/A5 to ESCON J5 pin 2 for Digital Input 2 enable; black wires provide the required signal/analog grounds.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/stm32-voltage-vertical-proof.jpg}
\caption{Voltage-isolation evidence from the STM32 debugging stage. The images show the inactive state near $0$ V, the analog RUN command at $1.00$ V, and the digital enable level at $3.31$ V.}
\end{figure}
...
\begin{enumerate}
  ...
  \item \textbf{Potentiometer motion test.} The command source was temporarily changed to Potentiometer 1. Digital Input 2 was enabled from the ESCON auxiliary connection through a removable jumper, and the potentiometer range was limited to approximately 0--100 rpm. This proved the ESCON, encoder, power wiring, and actuator could produce stable low-speed motion without involving STM32 code.
...
The deprecated Myo armband was connected to a laptop over BLE. The laptop subscribed to eight EMG channels and IMU data. The BLE address used in the command line identifies this specific armband during this setup; it is not a general address for all Myo devices. If the armband is sleeping, already connected to another computer/phone, or not advertising, the software can fail to connect even when the saved address is correct. For normal runs the known address was supplied directly, avoiding repeated broad Bluetooth scanning. For each $200$ ms sliding window with a $50$ ms step, the mean absolute value (MAV) was calculated:
\begin{equation}
...
The following table records what each evidence category supports. Representative photographs are embedded in the report, while the original videos and screenshots should be submitted in the companion evidence folder.
...
Potentiometer-controlled actuator video & ESCON auxiliary/jumper enable, encoder, supply, and PMA actuator could produce stable low-speed rotation before STM32 integration & Observed actuator motion only; no finger mechanism attached\\
...
\section{Companion media inventory}

The companion evidence folder should include the following transferred media files:
\begin{verbatim}
videos:
...
The report includes selected figures for readability. The original clips remain useful because they show the time sequence of potentiometer-controlled motion, timed STM32 output testing, and button-controlled actuator start/stop operation.
